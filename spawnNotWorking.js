...
let firstEmitRan = false
  ...
  async function input(command) {
   ...
   // outputArray.push(command)
    inputState = true  
    let data = null;
    console.log(SpawnOutput.name)
   
 
     

    inputState = false
    return  await termino.input(command);
  }//
  ...
  async function processInputData(inputData) {
    console.log("hll")
    if (typeof inputData === 'function' || typeof inputData === "Function") {
       let result;
      ...
      console.log(inputData.constructor.name)
  ...
  if (commands) {
    if(commands instanceof String === false){
    ...
    if(typeof commands === "Function" || commands.constructor.name === "AsyncFunction" || commands.constructor.name === "Function"){
      console.log("fguc")
       processInputData(commands);
    }
...
console.log("Spawn has been closed", data.message)
 ...
 terminal.addCommand('hello', (data) => {
console.log(data)
  });
