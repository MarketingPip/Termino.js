...
export function Termino(terminalSelector, keyCodes, settings) {
  ...
  try {
    ...
    if (settings) {
      ...
      function compare(json1, json2) {
        ...
        if (!json2.prompt.includes('{{command}}') || !json2.input.includes('{{command}}') || !json2.output.includes('{{command}}')) {
         throw {message: "Your overwritten Termino settings are not valid, {{command}} is required inside prompt, input & output."}
          return false
        }   
    ...
    // CHECK IF QUERY SELECTOR WAS PROVIDED (NON PROCRESS BASED ONLY)
    if(!terminalSelector){
  ...
  DEF_SETTINGS.speech_to_text = false
    ...
    if (key === KEYCODES[i].key_code) {
      ...
      if(KEYCODES[i].function){
        ...
        let ErrorMsg_KeyCode = `Termino.js: KeyCode Function Error: ${error.message}`
        console.error(ErrorMsg_KeyCode)
        throw {
                message: ErrorMsg_KeyCode
              }
      }
    }
  }
