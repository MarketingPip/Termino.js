...
let termInputResolve, termInputReject;
  ...
  InputState = true;
   const promise = new Promise((resolve, reject) => {
    ...
    promise.cancel = function () {
     if (InputState && typeof termInputReject === 'function') {
    termInputReject(new Error('Input cancelled'));
    InputState = false;
  }
  };
...
// console.log(data)
 ...
 term.addCommand("called", async (event) => {
 let b =  await term.input("helloss")
 console.log(b)
});
...
function emulate(strings){
  for(let item in strings){
    //console.log(item)//
   term.userInputSimulate(strings[item],null, false)   
  }
}
///
...
async function getInput() {
  try {
    input = term.input("Enter something: ");
    const reply = await input 
    return reply
  } catch (err) {
    console.log("Input was cancelled or rejected:", err.message);
  }
}
  
// To cancel the input:
// This starts the input process
