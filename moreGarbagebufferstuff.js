...
function disableTextToSpeech(){
  DEF_SETTINGS.speech_to_text = false
    ...
    function termInput(question, callback) {
      ...
      //  SpeechToText(question)
        ...
        function handleCommandForQuestion(event, READSTREAM = false, data = null) {
          InputState = true;
          if (event?.keyCode == Command_Key || READSTREAM) {
 ...
 if(!READSTREAM){           terminalSelector.querySelector(DEF_SETTINGS.terminal_input).removeEventListener('keypress', handleCommandForQuestion);
            ...
            let READSTREAM_INPUT = false
            if(data){
              READSTREAM_INPUT = true
            }
            
            if (value.length != 0) {
              // echo value to terminal
              echoHandler(value, callback, true, READSTREAM_INPUT)
              emit('data', value)
              getCallBack(value)
              resolve(value)
            } else {
              // return an empty prompt
              echoHandler("", callback, true, READSTREAM_INPUT)
              emit('data', null)
              getCallBack("")
              resolve()
            }
   ...
   READSTREAM = true     
  if(READSTREAM){        
   on('RADIOSTREAM_INPUT', (data) => {
    //console.log("Cant figure")
     if(!InputState){
       console.log("called wtf 2")
 ...
 console.log("wtff")   //
    ...
    function termEcho(command, callback = null) {
      echoHandler(command, callback, false, false)
    }
    
    // FUNCTION TO HANDLE TERM ECHOS (REQUIRED TO HANDLE .on(output))
   let READSTREAM_FIRST_EMIT = false;      

      function echoHandler(command, callback, userInput = true, READSTREAM_INPUT_VALUE = true){
        
         const CMD =  `${DEF_SETTINGS.prompt.replace('{{command}}', command)}`
      ...
      if(callback){
       ...
       if(!userInput && !READSTREAM_INPUT_VALUE){
         emit("output", command)
        terminal_console.innerHTML += CMD
       } 
        
         if(READSTREAM_INPUT_VALUE){
         //emit("output", command)
        terminal_console.innerHTML += CMD
       } 
      
      if(!userInput && !READSTREAM_FIRST_EMIT && READSTREAM_INPUT_VALUE){

       emit("RADIOSTREAM_INPUT", command)
        READSTREAM_FIRST_EMIT = true//
      } 
        
       if(!userInput && READSTREAM_INPUT_VALUE && READSTREAM_FIRST_EMIT){
      ...
      if (InputState != true) {
        ...
        if (key === Command_Key) {
         ...
         termValue = terminalSelector.querySelector(DEF_SETTINGS.terminal_input).value || terminalSelector.querySelector(DEF_SETTINGS.terminal_input).innerText  || terminalSelector.querySelector(DEF_SETTINGS.terminal_input).textContent
          ...
          echoHandler(termValue, null, false, false)
...
async function emulate(strings){
  for(let item in strings){
    console.log(item)////
   term.execAsUser(strings[item], null, false, true)
...
await emulate(['cool beans', 'called']) //
});




let data = await term.input("wtfs")
